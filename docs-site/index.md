...
Redline is a live parlay game built on TxLINE, the TxODDS sports-data oracle on Solana. Each player's car is a soccer parlay, and its position on the track is the live probability that the parlay still cashes, read directly from TxLINE's de-vigged `Pct` field.
...
- [Introduction](/guide/introduction): what Redline is, and the idea of reading a whole parlay as one moving number.
