...
A bet slip is a number that sits still until the final whistle. ComboRace makes that number a moving object. Every car on the track is a parlay, and its distance down the track is one thing: how likely that bet still pays right now.
...
TxODDS runs TxLINE, a verifiable sports-data oracle on Solana that ships live odds, scores, and match events, with results anchored on-chain. ComboRace uses that feed as its primary input: the odds place the cars, the scores resolve the legs, and the on-chain result settles the pool.
